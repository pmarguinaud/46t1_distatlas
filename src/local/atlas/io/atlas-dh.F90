...
FUNCTION ATLAS_DH_t_ctor (YDGRID) RESULT (THIS)

TYPE (ATLAS_STRUCTUREDGRID), INTENT (IN) :: YDGRID

TYPE (ATLAS_DH_t) :: THIS
REAL (KIND=JPRB)  :: ZBB (4), ZDM
...
ZBB = YDGRID%LONLATBOUNDINGBOX ()

THIS%ZLONW = ZBB (1); THIS%ZLONE = ZBB (2)
THIS%ZLATS = ZBB (3); THIS%ZLATN = ZBB (4)
...
IF (.NOT. YLRES%LLGLOBAL) THEN
  ...
  YLRES%ZLATN = 90.0_JPRB - YLRES%ZLATD * (YLRES%IROW1 - 0.5_JPRB)
  YLRES%ZLATS = 90.0_JPRB - YLRES%ZLATD * (YLRES%IROW2 - 0.5_JPRB)
...
BLOCK
  YLGRID = ATLAS_LONLATREGIONALGRID (YLRES%ICOLS, YLRES%IROWS, &
         & YLRES%ZLONW, YLRES%ZLONE, YLRES%ZLATN, YLRES%ZLATS)
ENDBLOCK
ENDIF
...
SELECT CASE (YLHDR%IWIDTH)
  ...
  CASE ( 8)
    ...
    IOFF = 0
    DO IROW = IROW1, IROW2
      READ (77, POS=1+(IROW-1)*IROWLENG+(ICOL1-1)*ICOLLENG) IL1
      WHERE (IL1 < 0)
  ...
  CASE (16)
    ...
    IOFF = 0
    DO IROW = IROW1, IROW2
      READ (77, POS=1+(IROW-1)*IROWLENG+(ICOL1-1)*ICOLLENG) IL2
