...
CALL GFA (CDFILE, YLGRID)
CALL YLGRID%RETURN ()

END FUNCTION

TYPE (ATLAS_FIELDSET) FUNCTION READ (THIS, CDFILE, CDNAME, YDSTCO) RESULT (YLFLST)
CLASS (ATLAS_FA_t), INTENT (IN) :: THIS
CHARACTER (LEN=*),  INTENT (IN) :: CDFILE
...
YLGRID = YDSTCO%GRID ()
CALL RFA (CDFILE, YLGRID, YDSTCO, CDNAME, YLFLST)
CALL YLGRID%FINAL ()
CALL YLFLST%RETURN ()
...
YLGRID = YDSTCO%GRID ()
CALL WFA (CDFILE, YLGRID, YDSTCO, YDFLST)
CALL YLGRID%FINAL ()
...
ZAHYBR = 0._JPRB
ZBHYBR = 0._JPRB 
INLOPA = [10_JPIM, 0_JPIM, 1_JPIM, INXLON, 1_JPIM, INLATI, 8_JPIM, 8_JPIM]
...
#define get(x,y) \
IF (.NOT. YLCONF%GET (#x, y)) CALL ABOR1 ('CANNOT FIND '//#x)
...
DO JFLD = 1, INFLD
  ...
  BLOCK
    ...
    CALL FAIENO (IREP, ILUN, CLPREF, INIVAU, CLSUFF, ZFLDG, .FALSE., &
               & LDUNDF=LLUNDF, PUNDF=ZUNDF, LDREVERT=.FALSE.)
...
SUBROUTINE RFA (CDFILENAME, YDGRID, YDSTCO, CDNOMA, YDFLSL)
...
INTEGER (KIND=JPIM) :: MYPROC, NPROC, IPROC
INTEGER (KIND=JPIM) :: JFLD1, JFLD2, INFLD, JFLD
INTEGER (KIND=JPIM) :: IFROM (SIZE (CDNOMA))
TYPE (ATLAS_FIELD)  :: YLFLDL (SIZE (CDNOMA))
TYPE (ATLAS_FIELD)  :: YLFLDG (SIZE (CDNOMA))
TYPE (ATLAS_FIELDSET) :: YLFLSG
TYPE (FCKIT_MPI_COMM) :: YLCOMM
...
! Distributed field : created by space function
  YLFLDL (JFLD) = YDSTCO%CREATE_FIELD (NAME=TRIM (CDNOMA (JFLD)), KIND=JPRB)
...
DO JFLD = 1, INFLD
  ...
  BLOCK
    ...
    WRITE (*, '("GLOBAL",A16,I16,E12.4)') TRIM (CLPREF (JFLD))//TRIM (CLSUFF (JFLD)), SIZE (ZFLDG), SUM (ZFLDG) / SIZE (ZFLDG)
  ENDBLOCK
  ENDIF
ENDDO
...
DO JFLD = 1, INFLD
  BLOCK
    TYPE (ATLAS_METADATA) :: YLMETA
    YLMETA = YLFLDL (JFLD)%METADATA ()
    IF (LLUNDEF (JFLD)) CALL YLMETA%SET ("undef", ZUNDEF (JFLD))
    CALL YLMETA%FINAL ()
  ENDBLOCK
ENDDO
...
BLOCK
  ...
  ELSE ! Regional lat/lon
    IF (ZLONE < ZLONW) THEN
      ZLONW = ZLONW - 360._JPRB
    ENDIF
    YDGRID = ATLAS_LONLATREGIONALGRID (ILONS, ILATS, ZLONW, ZLONE, ZLATS, ZLATN)
  ENDIF
  CALL YLCFGR%FINAL ()
  CALL YLCFDO%FINAL ()
ENDBLOCK
