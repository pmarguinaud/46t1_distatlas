...
BLOCK
  ...
  ELSE ! Regional lat/lon
    IF (ZLONE < ZLONW) THEN
      ZLONW = ZLONW - 360._JPRB
    ENDIF
    YLGRID = ATLAS_LONLATREGIONALGRID (ILONS, ILATS, ZLONW, ZLONE, ZLATS, ZLATN)
  ENDIF
  CALL YLCFGR%FINAL ()
  CALL YLCFDO%FINAL ()
ENDBLOCK
...
BLOCK ! Regional model
  ...
  YLGRID = ATLAS_LAMBERTREGIONALGRID (YLCADR%NXLOPA, YLCADR%NLATIT, -NUX / 2 * DXINMETRES, -NUY / 2 * DYINMETRES, &
...
! Global field; zero size if this proc does not read the field
  ISIZEG = 0
  IF (IFROM (JFLD) == MYPROC) ISIZEG = INT (YDSTCO%SIZE_OWNED (), JPIM)
