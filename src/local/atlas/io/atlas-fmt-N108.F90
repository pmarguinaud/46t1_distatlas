...
IF (THIS%IFP == 0) THEN
  CALL ATLAS_ABORT ('ATLAS_FMT_N108:OPEN CANNOT OPEN '//TRIM (CDFILE))
ENDIF
END SUBROUTINE OPEN
...
ELSEIF (LCMPM (CDNAME, 'Temp_s', IMONTH)) THEN
  IOFF8 = 5 + 3 * (IMONTH - 1)
ELSE
  CALL ATLAS_ABORT ('ATLAS_FMT_N108:READ: UNEXPECTED FIELD '//TRIM (CDNAME))
ENDIF
...
! Fortran unformatted control word
  ...
  IF (ILEN4 /= ILEN8) CALL ATLAS_ABORT ('ATLAS_FMT_N108:READ: UNEXPECTED SIZE')

! Read IEEE data
  CALL FI_FREAD (IERR8, PFLDG (1+(J-1)*ILON), ILEN8, 1_JPIB, THIS%IFP)
  IF (IERR8 /= 1) CALL ATLAS_ABORT ('ATLAS_FMT_N108:READ: UNEXPECTED END OF FILE')
...
CALL ATLAS_ABORT ('ATLAS_FMT_N108:WRITE: NOT IMPLEMENTED')
