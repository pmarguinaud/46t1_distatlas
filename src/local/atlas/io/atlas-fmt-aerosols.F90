...
999 CONTINUE
  ...
  CALL ABORT ('ATLAS_FMT_AEROSOLS:READSLSD: ERROR WHILE READING '//TRIM (CDFILE))
...
ELSEIF (LCMPM (CDNAME, 'desert', IMONTH)) THEN
  IND = 4
ELSE
  CALL ABORT ('ATLAS_FMT_AEROSOLS:READ: UNEXPECTED FIELD '//TRIM (CDNAME))
ENDIF
...
CALL ABORT ('ATLAS_FMT_AEROSOLS:WRITE NOT IMPLEMENTED')
