...
TYPE, EXTENDS (ATLAS_FMT_t) :: ATLAS_FMT_FA_t

  INTEGER (KIND=JPIM) :: ILUN = -1_JPIM
...
PUBLIC :: ATLAS_FMT_FA_t, ISMLAM, ISMGLO, ISLTLN

CONTAINS

TYPE (ATLAS_STRUCTUREDGRID) FUNCTION GRID (THIS, CDFILE) RESULT (YLGRID)
CLASS (ATLAS_FMT_FA_t), INTENT (IN) :: THIS
...
BLOCK ! Global model
  ...
  ELSE
    CALL ATLAS_ABORT ('UNEXPECTED ODD NUMBER OF LATITUDES')
  ENDIF
...
ELSEIF (LLLTLN) THEN
  CALL OFA_LTL
ELSE
  CALL ATLAS_ABORT ('UNKNOWN GEOMETRY: '//YLCONF%JSON ())
ENDIF
...
REAL (KIND=JPRB) :: ZLONLAT (2)
CHARACTER (LEN=:), ALLOCATABLE :: CLDOTY, CLNAME
...
IF (LLGLOBAL) THEN
  ZLOND = 360._JPRB / REAL (NX, JPRB)
  ZLONLAT = YDGRID%LONLAT (1, 1)
  ZLONW = ZLONLAT (1)
  IF (ZLONW > 0) ZLONW = ZLONW - 360._JPRB
  ZLONLAT = YDGRID%LONLAT (NX, 1)
  ZLONE = ZLONLAT (1)
  ZLONLAT = YDGRID%LONLAT (1, 1)
  ZLATN = ZLONLAT (2)
  ZLONLAT = YDGRID%LONLAT (1, NY)
  ZLATS = ZLONLAT (2)
  IF (ZLATS > ZLATN) CALL SWAP (ZLATS, ZLATN)
ELSE
  ...
  ENDIF
  ZLOND = MODULO (ZLONE-ZLONW, 360._JPRB) / REAL (NX-1, JPRB)
  LLFOUND = YLCONF%GET ('domain.ymin', ZLATS)
  LLFOUND = YLCONF%GET ('domain.ymax', ZLATN)
ENDIF

ZLATD = (ZLATN - ZLATS) / REAL (NY - 1, JPRB)

ZLONC = ZLONW + (ZLOND * NX) / 2._JPRB
ZLATC = ZLATS + (ZLATD * NY) / 2._JPRB
...
#define get(x,y) \
IF (.NOT. YLCONF%GET (#x, y)) CALL ATLAS_ABORT ('CANNOT FIND '//#x)
...
CLASS (ATLAS_FMT_FA_t),        INTENT (INOUT) :: THIS
CHARACTER (LEN=*),             INTENT (IN)    :: CDFILE
CLASS (ATLAS_STRUCTUREDGRID),  INTENT (IN)    :: YDGRID
...
! TODO : check geometry consistency for existing files
...
!   INGRIB = 0_JPIM
  ...
  THIS%LLMLAM = YLCADR%LIMLAM
  THIS%LLLTLN = YLCADR%SINLAT(2) < 0 .AND. THIS%LLMLAM
  THIS%LLMGLO = (.NOT. THIS%LLMLAM) .AND. (.NOT. THIS%LLLTLN)

  CALL FATCHA (IREP, CLNOMC, .FALSE., ILCHAM)

  IF (ILCHAM /= YDGRID%SIZE ()) CALL ATLAS_ABORT ('ATLAS_FMT_FA:OPEN: SIZE MISMATCH')

  THIS%LLREVERT = THIS%LLLTLN

  IF (THIS%LLLTLN) THEN
    BLOCK
      INTEGER (KIND=JPIM) :: NY
      REAL (KIND=JPRB) :: ZLONLAT (2), ZLATN, ZLATS
      NY = YDGRID%NY ()
      ZLONLAT = YDGRID%LONLAT (1, 1)
      ZLATN = ZLONLAT (2)
      ZLONLAT = YDGRID%LONLAT (1, NY)
      ZLATS = ZLONLAT (2)
      THIS%LLREVERT = ZLATS > ZLATN
    ENDBLOCK
  ELSE
    THIS%LLREVERT = .FALSE.
  ENDIF
...
CALL FAIRME (IREP, THIS%ILUN, 'KEEP')
THIS%ILUN = -1_JPIM
...
CALL FAQUIN (IREP, THIS%ILUN, CLPREF, INIVAU, CLSUFF, &
           & CDNAME, LEN_TRIM (CDNAME))
CALL FACILO (IREP, THIS%ILUN, CLPREF, INIVAU, CLSUFF, PFLDG, .FALSE., &
           & LDUNDF=LDUNDEF, PUNDF=PUNDEF, LDREVERT=.FALSE.)
...
! Parse field name
CALL FAQUIN (IREP, THIS%ILUN, CLPREF, INIVAU, CLSUFF, CDNAME, LEN_TRIM (CDNAME))

CALL FAIENO (IREP, THIS%ILUN, CLPREF, INIVAU, CLSUFF, PFLDG, &
           & .FALSE., LDUNDF=LDUNDEF, PUNDF=PUNDEF, &
           & LDREVERT=THIS%LLREVERT)
