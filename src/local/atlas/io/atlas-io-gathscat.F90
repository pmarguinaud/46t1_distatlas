...
TYPE, ABSTRACT, EXTENDS (ATLAS_IO_t) :: ATLAS_IO_GATHSCAT_t
CONTAINS

  PROCEDURE :: GRID
  PROCEDURE :: READ    
  PROCEDURE :: WRITE   
  PROCEDURE (FORMAT_f), DEFERRED, PASS (THIS) :: FORMAT
...
ABSTRACT INTERFACE
FUNCTION FORMAT_f (THIS) RESULT (YLFMT)
USE ATLAS_FMT, ONLY : ATLAS_FMT_t
IMPORT :: ATLAS_IO_GATHSCAT_t
IMPLICIT NONE
CLASS (ATLAS_IO_GATHSCAT_t), INTENT (IN), TARGET :: THIS
CLASS (ATLAS_FMT_t), POINTER :: YLFMT
END FUNCTION FORMAT_f
END INTERFACE
...
DO JFLD = 1, INFLD
  ...
  BLOCK
    ...
    CALL YLFMT%READ (CDNAME (JFLD), ZFLDG, LLUNDEF (JFLD), ZUNDEF (JFLD))
  ENDBLOCK
  ENDIF
ENDDO
...
IF (ANY (ITO == MYPROC)) THEN
  CALL YLFMT%OPEN (CLFILENAME, YLGRID)
ENDIF
...
DO JFLD = 1, INFLD
  ...
  BLOCK
    ...
    CALL YLFMT%WRITE (CLNOMA, ZFLDG, LLUNDEF, ZUNDEF)
