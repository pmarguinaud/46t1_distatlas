...
BLOCK
  TYPE (ATLAS_CONFIG) :: YLFSCONF
  YLFSCONF = ATLAS_CONFIG ()
  IF (LLHALFDIFF) THEN
    CALL YLFSCONF%SET ("periodic_points", .true.)
    CALL YLFSCONF%SET ("halo", 1)
  ENDIF
  YLFSSC1 = ATLAS_FUNCTIONSPACE_STRUCTUREDCOLUMNS (YLGRID1, YLDIST1, YLFSCONF)
  YLFSSC2 = ATLAS_FUNCTIONSPACE_STRUCTUREDCOLUMNS (YLGRID2, YLDIST2, YLFSCONF)
  CALL YLFSCONF%FINAL ()
ENDBLOCK
...
IF (LLHALFDIFF) YLFLDS1G = DO_HALFDIFF (YLFSSC1, YLFLDS1%FIELD ('X'))
...
IF (LLHALFDIFF) THEN
  CALL WFA ('HALFDIFF.fa', YLGRID1, YLFSSC1, YLFLDS1G)
ENDIF
