...
TYPE (ATLAS_FIELDSET)                        :: YLFLDS1G,YLFLDS2G
...
LOGICAL :: LLWRITE1, LLWRITE2, LLINTERP4, LLINTERPA, LLLIGHT1, LLLIGHT2, LLHALFDIFF
...
BLOCK
  TYPE (ATLAS_CONFIG) :: YLFSCONF1, YLFSCONF2
  YLFSCONF1 = ATLAS_CONFIG ()
  YLFSCONF2 = ATLAS_CONFIG ()
  IF (LLHALFDIFF) THEN
    CALL YLFSCONF1%SET ("halo", 1)
    CALL YLFSCONF1%SET ("periodic_points", .true.)
    CALL YLFSCONF2%SET ("halo", 1)
    CALL YLFSCONF2%SET ("periodic_points", .true.)
  ENDIF
  YLFSSC1 = ATLAS_FUNCTIONSPACE_STRUCTUREDCOLUMNS (YLGRID1, YLDIST1, YLFSCONF1)
  YLFSSC2 = ATLAS_FUNCTIONSPACE_STRUCTUREDCOLUMNS (YLGRID2, YLDIST2, YLFSCONF2)
  CALL YLFSCONF1%FINAL ()
  CALL YLFSCONF2%FINAL ()
ENDBLOCK
...
IF (LLHALFDIFF) THEN
    YLFLDS1G = DO_HALFDIFF (YLFSSC1, YLFLDS1%FIELD ('X'))
    YLFLDS2G = DO_HALFDIFF (YLFSSC2, YLFLDS2%FIELD ('X'))
ENDIF
...
IF (LLHALFDIFF) THEN
  CALL WFA ('HALFDIFF1.fa', YLGRID1, YLFSSC1, YLFLDS1G)
  CALL WFA ('HALFDIFF2.fa', YLGRID2, YLFSSC2, YLFLDS2G)
ENDIF
...
DO IY = YDFSSC%J_BEGIN (), YDFSSC%J_END ()
  DO IX = YDFSSC%I_BEGIN (IY), YDFSSC%I_END (IY)
    ...
    IF (.TRUE.) THEN
        LONLATTMP=YDGRID%LONLAT (YDFSSC%I_BEGIN (IY),IY)
        TMP1=LONLATTMP(1) * DEG2RAD
        LONLATTMP=YDGRID%LONLAT (YDFSSC%I_END (IY),IY)        
        TMP2=LONLATTMP(1) * DEG2RAD
        LONLAT(1)=R2PI*(LONLAT(1)-TMP1)/(TMP2-TMP1)
        LONLATTMP=YDGRID%LONLAT (IX,YDFSSC%J_BEGIN ())
        TMP1=LONLATTMP(2) * DEG2RAD
        LONLATTMP=YDGRID%LONLAT (IX,YDFSSC%J_END ())
        TMP2=LONLATTMP(2) * DEG2RAD
        LONLAT(2)=R2PI*(LONLAT(2)-TMP1)/(TMP2-TMP1)
    ENDIF
    WRITE(*,*) LONLAT (1),LONLAT (2)
