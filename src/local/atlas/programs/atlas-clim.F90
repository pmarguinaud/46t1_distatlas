...
INTEGER (KIND=JPIM) :: JSURFGZ0THERM   , JSURFZ0VEGFOISG , JSURFALBEDOVEG  , JSURFPROPVEGETAT
...
CALL YLFAIO%WRITE ('INCLI9', YLFSG2, YLFSSC2)

DO IMONTH = 1, 12
  BLOCK
    TYPE (ATLAS_FIELD) :: YLF
    WRITE (CLMONTH, '(".m",I2.2)') IMONTH
    YLF = YLFSG2%FIELD ("sea"   //CLMONTH); CALL YLF%RENAME ("SURFAEROS.SEA"   ); CALL INS2 (JSURFAEROSSEA   , YLFL2M (:, IMONTH), YLPT2M (:, IMONTH), YLF); CALL YLF%FINAL ()
    YLF = YLFSG2%FIELD ("land"  //CLMONTH); CALL YLF%RENAME ("SURFAEROS.LAND"  ); CALL INS2 (JSURFAEROSLAND  , YLFL2M (:, IMONTH), YLPT2M (:, IMONTH), YLF); CALL YLF%FINAL ()
    YLF = YLFSG2%FIELD ("soot"  //CLMONTH); CALL YLF%RENAME ("SURFAEROS.SOOT"  ); CALL INS2 (JSURFAEROSSOOT  , YLFL2M (:, IMONTH), YLPT2M (:, IMONTH), YLF); CALL YLF%FINAL ()
    YLF = YLFSG2%FIELD ("desert"//CLMONTH); CALL YLF%RENAME ("SURFAEROS.DESERT"); CALL INS2 (JSURFAEROSDESERT, YLFL2M (:, IMONTH), YLPT2M (:, IMONTH), YLF); CALL YLF%FINAL ()
  ENDBLOCK
ENDDO
...
DO IMONTH = 1, 12
  BLOCK
    TYPE (ATLAS_FIELD) :: YLF
    WRITE (CLMONTH, '(".m",I2.2)') IMONTH
    YLF = YLFSG2%FIELD ("a"//CLMONTH); CALL YLF%RENAME ("SURFA.OF.OZONE"); CALL INS2 (JSURFAOFOZONE, YLFL2M (:, IMONTH), YLPT2M (:, IMONTH), YLF); CALL YLF%FINAL ()
    YLF = YLFSG2%FIELD ("b"//CLMONTH); CALL YLF%RENAME ("SURFB.OF.OZONE"); CALL INS2 (JSURFBOFOZONE, YLFL2M (:, IMONTH), YLPT2M (:, IMONTH), YLF); CALL YLF%FINAL ()
    YLF = YLFSG2%FIELD ("c"//CLMONTH); CALL YLF%RENAME ("SURFC.OF.OZONE"); CALL INS2 (JSURFCOFOZONE, YLFL2M (:, IMONTH), YLPT2M (:, IMONTH), YLF); CALL YLF%FINAL ()
  ENDBLOCK
ENDDO
...
DO IMONTH = 1, 12
  BLOCK
    ...
    CALL INS2 (JSURFPROPVEGETAT, YLFL2M (:, IMONTH), YLPT2M (:, IMONTH), CPYFLD (YLFL2 (JPROPVEGETAT)))
...
!  Vegetation fraction (corrected with land fraction)
      ...
      ZV  = MIN (YLPT2 (JPROPVEGETAT)%ZDATA (JLOC2), ZVI * YLPT2 (JPROPTERRE)%ZDATA (JLOC2))
...
!  Set values over sea (no vegetation, albedo and roughness unchanged)
      ...
      IF (IV == NTPMER .OR. IV == NTPLAC) THEN
        YLPT2M (JSURFPROPVEGETAT, IMONTH)%ZDATA (JLOC2) = 0.0_JPRB
        YLPT2M (JSURFINDFOLIAIRE, IMONTH)%ZDATA (JLOC2) = 0.0_JPRB
        YLPT2M (JSURFRESISTOMIN , IMONTH)%ZDATA (JLOC2) = SRSMX
        YLPT2M (JSURFGZ0THERM   , IMONTH)%ZDATA (JLOC2) = YLPT2 (JZ0FOISG)%ZDATA (JLOC2)
        YLPT2M (JSURFZ0VEGFOISG , IMONTH)%ZDATA (JLOC2) = 0.0_JPRB
        YLPT2M (JSURFALBEDOVEG  , IMONTH)%ZDATA (JLOC2) = SALBN
      ELSE
...
!  contributions from vegetation, urban areas and desert ; scaling
        ZU = YLPT2 (JPROPURBANIS)%ZDATA (JLOC2)

        Z0 = ZFZ * (ZV * Z0 + ZU * SZZ0U + (1.0_JPRB - ZU - ZV) * SZZ0D)
    
!  Final values
        
        YLPT2M (JSURFPROPVEGETAT, IMONTH)%ZDATA (JLOC2) = ZV
        YLPT2M (JSURFZ0FOISG    , IMONTH)%ZDATA (JLOC2) = SQRT (YLPT2 (JZ0FOISG)%ZDATA (JLOC2)**2 + Z0**2)
        YLPT2M (JSURFALBEDO     , IMONTH)%ZDATA (JLOC2) = MIN (SALBX, MAX (SALBN, (1.0_JPRB - ZV) * YLPT2 (JALBEDO)%ZDATA (JLOC2) + ZV * ZA))
        YLPT2M (JSURFINDFOLIAIRE, IMONTH)%ZDATA (JLOC2) = ZL
        YLPT2M (JSURFRESISTOMIN , IMONTH)%ZDATA (JLOC2) = MIN (SRSMX, MAX (SRSMN, ZR))
        YLPT2M (JSURFGZ0THERM   , IMONTH)%ZDATA (JLOC2) = Z0 * STHER

        YLPT2M (JSURFZ0VEGFOISG , IMONTH)%ZDATA (JLOC2) = Z0 
        YLPT2M (JSURFALBEDOVEG  , IMONTH)%ZDATA (JLOC2) = ZA
      ENDIF
