...
CALL INS (JZS     , DIR (CLDATA (JOROG_DAT), YLHDRS (JOROG_DAT), YLFSSC1, YLGRID1, YLDIST1), 1_JPIM)
CALL INS (JCLAY   , DIR (CLDATA (JCLAY_DAT), YLHDRS (JCLAY_DAT), YLFSSC1, YLGRID1, YLDIST1, 0.01_JPRB), 1_JPIM)
CALL INS (JSAND   , DIR (CLDATA (JSAND_DAT), YLHDRS (JSAND_DAT), YLFSSC1, YLGRID1, YLDIST1, 0.01_JPRB), 1_JPIM)
CALL INS (JCOVER+1, DIR (CLDATA (JCOVR_DAT), YLHDRS (JCOVR_DAT), YLFSSC1, YLGRID1, YLDIST1), 1_JPIM)
...
BLOCK
  ...
  DO ICOV = 1, INCOVR
    ...
    CALL INS (JCOVER+ICOV, NEWFLD (YLFSSC2, TRIM (CLNOMA)), 2)
...
BLOCK
  ...
  CALL INS (JZS2    , NEWFLD (YLFSSC1, "SFX.ZS_2"    , ZUNDEF), 1)
  CALL INS (JZS_DXDX, NEWFLD (YLFSSC1, "SFX.ZS.DX_DX", ZUNDEF), 1)
  CALL INS (JZS_DYDY, NEWFLD (YLFSSC1, "SFX.ZS.DY_DY", ZUNDEF), 1)
  CALL INS (JZS_DXDY, NEWFLD (YLFSSC1, "SFX.ZS.DX_DY", ZUNDEF), 1)
...
CALL INS (JZS  , YLFLDS2I%FIELD ("SFX.ZS"  ), 2)
CALL INS (JSAND, YLFLDS2I%FIELD ("SFX.SAND"), 2)
CALL INS (JCLAY, YLFLDS2I%FIELD ("SFX.CLAY"), 2)

! Min, max, avg orography

BLOCK
 
  CALL INS (JAVG_ZS, NEWFLD (YLFSSC2, "SFX.AVG_ZS", ZUNDEF), 2)
  CALL INS (JSIL_ZS, NEWFLD (YLFSSC2, "SFX.SIL_ZS", ZUNDEF), 2)

  YLPT2 (JAVG_ZS)%ZDATA = YLPT2 (JZS)%ZDATA
  YLPT2 (JSIL_ZS)%ZDATA = YLPT2 (JZS)%ZDATA

  CALL INS (JMIN_ZS, YLINTEA%INTERPOLATE (YLFL1 (JZS), YLINTEA%OPT_MIN ()), 2)
  CALL INS (JMAX_ZS, YLINTEA%INTERPOLATE (YLFL1 (JZS), YLINTEA%OPT_MAX ()), 2)
...
BLOCK
  ...
  CALL INS (JZS2    , YLFLDS2I%FIELD ("SFX.ZS_2"    ), 2)
  CALL INS (JZS_DXDX, YLFLDS2I%FIELD ("SFX.ZS.DX_DX"), 2)
  CALL INS (JZS_DYDY, YLFLDS2I%FIELD ("SFX.ZS.DY_DY"), 2)
  CALL INS (JZS_DXDY, YLFLDS2I%FIELD ("SFX.ZS.DX_DY"), 2)

  CALL INS (JSSO_DIR,   NEWFLD (YLFSSC2, "SFX.SSO_DIR"  , ZUNDEF), 2)
  CALL INS (JSSO_SLOPE, NEWFLD (YLFSSC2, "SFX.SSO_SLOPE", ZUNDEF), 2)
  CALL INS (JSSO_ANIS , NEWFLD (YLFSSC2, "SFX.SSO_ANIS" , ZUNDEF), 2)
  CALL INS (JSSO_STDEV, NEWFLD (YLFSSC2, "SFX.SSO_STDEV", ZUNDEF), 2)
...
BLOCK
  ...
  TYPE (ATLAS_FIELD) :: YLOROG1INT4, YLOROG1IP, YLOROG1IM, &
                      & YLOROG1JP, YLOROG1JM, YLOROG1MINT4
  REAL (KIND=JPRB), POINTER, DIMENSION (:) :: ZOROG1INT4, &
                   & ZOROG1IP, ZOROG1IM, ZOROG1JP, ZOROG1JM, ZOROG1MINT4
...
!$OMP END WORKSHARE
  ...
  CALL YLFLDSHALF%ADD (YLOROG1INT4)

  YLFLDSAOS1 = HALFDIFF (YLFSSC1, YLFLDSHALF)

  YLOROG1IP = YLFLDSAOS1%FIELD ("SFX.ZS.XP"); CALL YLOROG1IP  %DATA (ZOROG1IP)
  YLOROG1IM = YLFLDSAOS1%FIELD ("SFX.ZS.XM"); CALL YLOROG1IM  %DATA (ZOROG1IM)
  YLOROG1JP = YLFLDSAOS1%FIELD ("SFX.ZS.YP"); CALL YLOROG1JP  %DATA (ZOROG1JP)
  YLOROG1JM = YLFLDSAOS1%FIELD ("SFX.ZS.YM"); CALL YLOROG1JM  %DATA (ZOROG1JM)

!$OMP WORKSHARE
  WHERE (ZOROG1IP /= ZUNDEF) ZOROG1IP = ZOROG1IP / RA
!$OMP END WORKSHARE
!$OMP WORKSHARE
  WHERE (ZOROG1IM /= ZUNDEF) ZOROG1IM = ZOROG1IM / RA
!$OMP END WORKSHARE
!$OMP WORKSHARE
  WHERE (ZOROG1JP /= ZUNDEF) ZOROG1JP = ZOROG1JP / RA
!$OMP END WORKSHARE
!$OMP WORKSHARE
  WHERE (ZOROG1JM /= ZUNDEF) ZOROG1JM = ZOROG1JM / RA
!$OMP END WORKSHARE


  CALL YLFLDSAOS1%FINAL ()

  CALL INS (JAOSIP, NEWFLD (YLFSSC1, "SFX.AOSIP", ZUNDEF), 1)
  CALL INS (JAOSIM, NEWFLD (YLFSSC1, "SFX.AOSIM", ZUNDEF), 1)
  CALL INS (JAOSJP, NEWFLD (YLFSSC1, "SFX.AOSJP", ZUNDEF), 1)
  CALL INS (JAOSJM, NEWFLD (YLFSSC1, "SFX.AOSJM", ZUNDEF), 1)

!$OMP WORKSHARE
  WHERE (YLPT1 (JZS)%ZDATA /= ZUNDEF)
    YLPT1 (JAOSIP)%ZDATA = + MAX (0._JPRB, ZOROG1IP)
    YLPT1 (JAOSIM)%ZDATA = - MIN (0._JPRB, ZOROG1IM)   
    YLPT1 (JAOSJP)%ZDATA = + MAX (0._JPRB, ZOROG1JP)
    YLPT1 (JAOSJM)%ZDATA = - MIN (0._JPRB, ZOROG1JM)
  ELSEWHERE
...
!$OMP END WORKSHARE
  ...
  CALL INS (JHO2IP, NEWFLD (YLFSSC1, "SFX.HO2IP", ZUNDEF), 1)
  CALL INS (JHO2IM, NEWFLD (YLFSSC1, "SFX.HO2IM", ZUNDEF), 1)
  CALL INS (JHO2JP, NEWFLD (YLFSSC1, "SFX.HO2JP", ZUNDEF), 1)
  CALL INS (JHO2JM, NEWFLD (YLFSSC1, "SFX.HO2JM", ZUNDEF), 1)
...
!$OMP END WORKSHARE
  ...
  CALL YLFLDSAOS1%ADD (YLFL1 (JAOSIP)); CALL YLFLDSAOS1%ADD (YLFL1 (JAOSIM))
  CALL YLFLDSAOS1%ADD (YLFL1 (JAOSJP)); CALL YLFLDSAOS1%ADD (YLFL1 (JAOSJM))
  CALL YLFLDSAOS1%ADD (YLFL1 (JHO2IP)); CALL YLFLDSAOS1%ADD (YLFL1 (JHO2IM))
  CALL YLFLDSAOS1%ADD (YLFL1 (JHO2JP)); CALL YLFLDSAOS1%ADD (YLFL1 (JHO2JM))
  ...
  DO JFLD = 1, YLFLDSAOS2%SIZE ()
    BLOCK
      ...
      YLFLD = YLFLDSAOS2%FIELD (JFLD)
      CALL YLFLD%DATA (ZDATA)
!$OMP WORKSHARE
      WHERE ((YLPT2 (JZS)%ZDATA /= ZUNDEF) .AND. (ZDATA == ZUNDEF))
        ZDATA = 0._JPRB
      ENDWHERE
!$OMP END WORKSHARE
      CALL YLFLDS2%ADD (YLFLD)
      CALL YLFLD%FINAL ()
    ENDBLOCK
  ENDDO
...
BLOCK
  ...
  CALL INS (JFRAC_SEA   , NEWFLD (YLFSSC2, "SFX.FRAC_SEA"   , ZUNDEF), 2)
  CALL INS (JFRAC_NATURE, NEWFLD (YLFSSC2, "SFX.FRAC_NATURE", ZUNDEF), 2)
  CALL INS (JFRAC_TOWN  , NEWFLD (YLFSSC2, "SFX.FRAC_TOWN"  , ZUNDEF), 2)
  CALL INS (JFRAC_WATER , NEWFLD (YLFSSC2, "SFX.FRAC_WATER" , ZUNDEF), 2)
...
BLOCK
  ...
  CALL INS (JBATHY  , NEWFLD (YLFSSC2, "SFX.BATHY"    , ZUNDEF), 2)
  CALL INS (JRUNOFFB, NEWFLD (YLFSSC2, "SFX.RUNOFF"   , ZUNDEF), 2)
  CALL INS (JWDRAIN , NEWFLD (YLFSSC2, "SFX.WDRAIN"   , ZUNDEF), 2)
...
SUBROUTINE INS (KRANK, YDFL, KGRID)

INTEGER (KIND=JPIM), INTENT (IN) :: KRANK, KGRID
