...
IF (.NOT. (GRID_EQ (YLGRID_OROG, YLGRID_CLAY))) CALL ATLAS_ABORT ("GEOMETRY MISMATCH")
IF (.NOT. (GRID_EQ (YLGRID_OROG, YLGRID_SAND))) CALL ATLAS_ABORT ("GEOMETRY MISMATCH")
...
ADDFLD (JSSO_DIR    ); ADDFLD (JSSO_SLOPE  ); ADDFLD (JSSO_ANIS   ); 
ADDFLD (JSSO_STDEV  ); ADDFLD (JAOSIP      ); ADDFLD (JAOSIM      ); 
ADDFLD (JAOSJP      ); ADDFLD (JAOSJM      ); ADDFLD (JHO2IP      ); 
ADDFLD (JHO2IM      ); ADDFLD (JHO2JP      ); ADDFLD (JHO2JM      );
ADDFLD (JFRAC_SEA   ); ADDFLD (JFRAC_TOWN  ); ADDFLD (JFRAC_WATER ); 
ADDFLD (JFRAC_NATURE); ADDFLD (JBATHY      ); ADDFLD (JRUNOFFB    ); 
ADDFLD (JWDRAIN     ); ADDFLD (JMIN_ZS     ); ADDFLD (JMAX_ZS     ); 
ADDFLD (JAVG_ZS     ); ADDFLD (JSIL_ZS     );
...
BLOCK
  ...
  YLFLDS2SSO = ATLAS_COMPUTE_SSO (YLFSSC1, YLFSSC2, YLOROG1, YLFL2 (JZS), YLINTEA, LLOPENMP, LLROTATE) 
...
IF (.NOT. YLFL2 (KRANK)%IS_NULL ()) CALL ATLAS_ABORT ('FIELD ALREADY EXISTS')
