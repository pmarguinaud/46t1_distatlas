...
LOGICAL, PARAMETER :: LLOPENMP = .TRUE.
...
IF (.NOT. GRID_EQ (YLGRID1PRE, YLGRID1PGD)) THEN
  CALL ATLAS_ABORT ('GEOMETRY MISMATCH')
ENDIF
...
YLINTEA = INTERPOLATIONA (YLDIST1, YLFSSC1, YLDIST2, YLFSSC2, LLOPENMP)
...
BLOCK
  ...
  DO JFLD = 1, YLFLDS2PRE%SIZE ()
    ...
    IF (YLFLDA%NAME () /= YLFLD4%NAME ()) &
      & CALL ATLAS_ABORT ('FIELD MISMATCH')
