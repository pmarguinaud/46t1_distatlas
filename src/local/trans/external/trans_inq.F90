...
IF(PRESENT(K_REGIONS_NS))   K_REGIONS_NS = N_REGIONS_NS
IF(PRESENT(K_REGIONS_EW))   K_REGIONS_EW = N_REGIONS_EW
IF(PRESENT(LDLAM))       LDLAM      = G%LAM
IF(PRESENT(KDEF_RESOL))  KDEF_RESOL = NDEF_RESOL 

IF (PRESENT (K_REGIONS)) THEN
   IF (SIZE (K_REGIONS) < N_REGIONS_NS) THEN
     CALL ABORT_TRANS('TRANS_INQ: K_REGIONS TOO SMALL')
   ENDIF
...
IF(PRESENT(KLOEN)) THEN
  IF(UBOUND(KLOEN,1) < R%NDGL) THEN
    CALL ABORT_TRANS('TRANS_INQ: KLOEN DIM 1 TOO SMALL')
  ELSE
    KLOEN(1:R%NDGL) = G%NLOEN(:)
  ENDIF
ENDIF
