...
!                       & ATLAS_STRUCTUREDGRID,ATLAS_FUNCTIONSPACE_STRUCTUREDCOLUMNS
...
SELECT CASE (TRIM (CLPROJ))
  ...
  CASE DEFAULT
    CALL OFA_GLO
    !FLOCALL ABOR1 ('Unknown geometry:'//YLCONF%JSON ())
END SELECT
...
SUBROUTINE WFATLAS (CDFILENAME, YDGRID, YDNDCO, CDPREF, KNIVAU, CDSUFF, YDFLSL)

USE FCKIT_MODULE, ONLY : FCKIT_MPI_COMM

CHARACTER (LEN=*),     INTENT (IN) :: CDFILENAME
CLASS (ATLAS_STRUCTUREDGRID),           INTENT (IN) :: YDGRID
TYPE (ATLAS_FUNCTIONSPACE_STRUCTUREDCOLUMNS), INTENT (IN) :: YDNDCO
...
! Set owner
  ...
  CALL YLMETA%FINAL ()
  CALL YDNDCO%GATHER (YLFLDL(JFLD), YLFLDG (JFLD))
  CALL YLFLSG%ADD (YLFLDG (JFLD))

ENDBLOCK
ENDDO

!CALL YDNDCO%GATHER (YDFLSL, YLFLSG)
...
! Distributed field : created by space function
  YLFLDL (JFLD) = YDNDCO%CREATE_FIELD (NAME=CLNOMA, KIND=JPRB)
...
CALL YDNDCO%SCATTER (YLFLSG, YDFLSL)
...
WRITE (*, *) __FILE__, ':', __LINE__, " -NUX / 2 * DXINMETRES = ", &
& -NUX / 2 * DXINMETRES
WRITE (*, *) __FILE__, ':', __LINE__, " -NUY / 2 * DYINMETRES = ", &
& -NUY / 2 * DYINMETRES
WRITE (*, *) -NUX / 2 * DXINMETRES + DXINMETRES * YLCADR%NXLOPA
WRITE (*, *) -NUY / 2 * DYINMETRES + DYINMETRES * YLCADR%NLATIT
