...
! Global field; zero size if this proc does not read the field
  ...
  YLFLDG (JFLD) = ATLAS_FIELD (NAME=TRIM (CLNOMA), KIND=JPRB, SHAPE=[ISIZEG])
...
! Distributed field : created by space function
  YLFLDL (JFLD) = YDSTCO%CREATE_FIELD (NAME=TRIM (CLNOMA), KIND=JPRB)
  CALL YLFLDL (JFLD)%DATA (ZFLDL)
  ZFLDL = 0._JPRB ! Not necessary
  YLMETA = YLFLDG (JFLD)%METADATA ()
  CALL YLMETA%SET ("CLPREF", TRIM (CDPREF (JFLD)))
  CALL YLMETA%SET ("INIVAU", KNIVAU (JFLD))
  CALL YLMETA%SET ("CLSUFF", TRIM (CDSUFF (JFLD)))
  CALL YLMETA%FINAL ()
